...
실전 운영: 2026-02-20~ (Hyperliquid 메인넷, 4 에이전트)
```
...
- [x] **스크리너 스케줄러** (`src/pipeline/screener_scheduler.py`): 72시간 주기 자동 코인 선별 + 심볼 풀 갱신
- [x] **대시보드** (`src/dashboard/app.py`): 웹 대시보드 (:8501) — 포지션, 에쿼티, 에이전트 상태
- [x] **텔레그램 양방향 대화** (`src/notify/telegram.py`): system 토픽에서 자연어 질문 → AI 답변 (잔고, 포지션, 수익, 상태 조회)
- [x] **매매 일시중지/재개**: Telegram에서 "매매 중지"/"매매 재개" 명령 지원
- [x] **모니터링 봇** (`scripts/monitor.py`): 5분 간격 — 프로세스 생존, 로그 갱신, 크리티컬 에러, 잔고 급감 감지
- [x] **동적 RR 팩터** (`phase5_execute.py`): 6개 시장 조건(추세정렬, 신뢰도, MTF, 스캔점수, MDD, 연패) 가중합산 → TP 공격/보수 자동 조절
...
> 우선순위: P0(필수) → P1(중요) → P2(개선) → P3(장기)

---

## P0: 안정성 + 운영 필수

### 1. VPS 마이그레이션 (24/7 가동)
- 현재: 로컬 맥에서 실행 → sleep/SIGTERM으로 봇 종료 빈발
- 목표: 클라우드 서버(Hetzner CX22 등)에서 systemd 또는 Docker로 24/7 운영
- 작업: 서버 셋업, .env 이전, Docker Compose 배포, 헬스체크 외부 모니터링

### 2. 프로세스 자동 재시작
- 현재: 봇 죽으면 수동 재시작
- 목표: 크래시 시 자동 복구 (systemd Restart=always 또는 supervisor)
- 포함: 포트 충돌 방지 (graceful shutdown 시 소켓 해제 보장)

### 3. Heartbeat 로깅
- 현재: 캔들 close 사이 5분간 로그 없음 → 모니터가 오탐
- 목표: 60초마다 heartbeat INFO 로그 → 모니터 오탐 방지
- 작업: main.py 또는 runner.py에 periodic heartbeat task 추가
...
### 4. 성과 어트리뷰션 분석
- 어떤 패턴(T1~T8)이 수익에 기여하는지 정량 분석
- 어떤 시장 레짐에서 성과가 좋은지 통계
- 결과를 Evolver 입력으로 연결 → 약한 패턴 자동 가중치 하향

### 5. Walk-Forward 백테스트
- 현재: 고정 파라미터로 전체 기간 백테스트 (과적합 위험)
- 목표: 3개월 학습 → 1개월 검증 rolling window 방식
- Out-of-sample 성과로 파라미터 신뢰도 평가

### 6. 에이전트 간 자본 재배분
- 현재: 고정 비율 (S1:15%, S2:25%, S3:30%, S4:30%)
...
### 9. 오더북 깊이 분석
- 현재: 스프레드만 확인
- 목표: 오더북 imbalance, 대형 벽(wall) 감지 → 진입/청산 타이밍 개선
- Phase 4 Gate 점수에 sub-score 추가

### 10. 슬리피지 실측 + 피드백
- 실제 체결가 vs 시그널 진입가 차이 기록
- 심볼별 평균 슬리피지 → 백테스트 비용 모델 자동 보정
- 슬리피지 큰 코인은 스크리너에서 페널티

### 11. 부분 청산 개선
- 현재: TP1에서 60% 한 번에 청산
- 목표: 3단계 TP (40/30/30) 옵션 + 각 단계별 트레일링 조건 분리
...
### 12. 멀티 거래소 지원
- Hyperliquid 외 Binance Futures, Bybit 등 추가
- 거래소별 어댑터 패턴으로 executor 추상화
- 거래소 간 아비트라지 또는 리스크 분산

### 13. Monte Carlo 시뮬레이션
- 백테스트 결과에서 거래 순서를 무작위 셔플 → 1000회 시뮬레이션
- 최악 시나리오 MDD, 파산 확률 추정
- 적정 자본금 산출

### 14. 웹/모바일 관리 앱
- 현재: Telegram 텍스트 기반
- 목표: 웹앱 또는 모바일 앱으로 실시간 모니터링 + 관리
- 차트, 포지션, PnL, 에이전트 상태 시각화

### 15. 온체인 데이터 통합
- DEX 거래량, 고래 지갑 이동, 스테이블코인 유입/유출
- Phase 2 DNA에 온체인 지표 추가
